...
<%
	...
	if rstDespesa.eof then
		...
		response.end
'	elseif rstDespesa.eof then
'		response.write(rstDespesa.eof)
	end if
...
response.end
		...
		do while not rstDespesas.EOF
			...
			sqlGrava = sqlGrava & "values (" & rstDespesas("cod_compromisso") & "," & competencia & "," & replace(rstDespesas("valor_previsto"),",",".") & ")"
